...
final class SourceKitObfuscatorTests: XCTestCase {
    func test_moduleRegistration() throws {
        ...
        let obfuscator = SourceKitObfuscator(sourceKit: sourceKit, logger: DummyLogger(), dataStore: dataStore, namesToIgnore: [], ignorePublic: false, modulesToIgnore: [], fileNamesToIgnore: [])
        ...
        XCTAssertEqual(Set(dataStore.indexedFiles.map(\.file)), module.sourceFiles)
    }
